...
std::string ClockCalendar::Print() {
    ...
    return str("Date: ") + std::to_string(m_day) + str("/") + std::to_string(m_month) + str("/") + std::to_string(m_year) + str("\nTime: ") + std::to_string(m_hr) + str("h") + std::to_string(m_min) + str("m") + std::to_string(m_sec) + str("s") + (m_isPM ? "PM" : "AM");
}
